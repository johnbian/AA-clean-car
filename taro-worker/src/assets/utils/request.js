...
async function doRequest ({baseUrl = setting.apiUrl ,url, method = 'GET', 
data = null, needLoading = true}) {
  let loadingShow = null
  if (needLoading) {
    loadingShow = await Taro.showLoading({title: ''})
  }
  const header = setting.openid ? { 'content-type': 'application/json', 'openid': setting.openid }
  :  { 'content-type': 'application/json' }
  if (!needLoading || loadingShow) {
    let res
    if (data) {
